...
//GET PRODUCTCODE BY $GET FROM ANOTHER FORM(ANOTHER FILE) THAN USE FUNCTION TO GET PRODUCT DETAILS.
$id =substr($_GET["update"],5);//like explode update45 so 45 will be the id
$productDetails = getProdByProdId($id);
//details:
$prodname =$productDetails['prodname']; 
$prodpicture =$productDetails['prodpicture'];
$categoryid = $productDetails['categoryid'];
$userid = $productDetails['userid'];
$price = $productDetails['price'];
$prodcondition = $productDetails["prodcondition"];
$desc = $productDetails["description"];

if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    $prodname=trim(filter_input(INPUT_POST,"prodname",FILTER_SANITIZE_SPECIAL_CHARS));
    $categoryid=trim(filter_input(INPUT_POST,"category",FILTER_SANITIZE_SPECIAL_CHARS));
    $price=trim(filter_input(INPUT_POST,"price",FILTER_SANITIZE_NUMBER_FLOAT));
    $prodcondition=trim(filter_input(INPUT_POST,"condition",FILTER_SANITIZE_SPECIAL_CHARS));
    $desc=trim(filter_input(INPUT_POST,"desc",FILTER_SANITIZE_SPECIAL_CHARS));
    
    if(empty($prodname) || empty($desc) || $price<0 || empty($price) ||$categoryid=="empty" ||$prodcondition=="empty1")//check errors
    {
        $err='<script>swal({
        title: "Error",
             text: "Empty inputs",
   ...
   if (isset($_FILES['imgupldr'])) //check if we have pic
       {
            ...
            $rc = updateP($prodname,$categoryid,$price,$prodcondition,$desc,$prodpicture,$userid,$id);
            if ($rc == 0) {
                $err='<script>swal({
        title: "Error",
             text: "Updateing product failed",
...
<html>
    ...
    <div class="container mb-5">
            <div class="col-sm-9 col-md-7 col-lg-5 mx-auto">
                <br><br><br>
            <img class="mb-4" src="https://image.flaticon.com/icons/svg/457/457710.svg" alt="" width="90" height="90">
            <hr class="my-4">
            <h2 class="text-warning bold font-weight-bold">Update Product</h2>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" readonly="readonly" placeholder="Product ID" style="text-align: center"  value="<?= $id?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" id="price" placeholder="Price" style="text-align: center" name="price" value="<?=$price?>">
                </div>
                <div class="form-group">
                ...
                <button type="submit" class="btn btn-lg btn-warning btn-block">Update Now ! </button>
